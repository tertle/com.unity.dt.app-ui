...
namespace Unity.AppUI.Samples.Redux
{
    public static class Reducers
    {
        ...
        public static CounterState Increment(CounterState state, IAction action)
        {
            return state with { value = state.value + 1 };
        }

        public static CounterState Decrement(CounterState state, IAction action)
        {
            return state with { value = state.value - 1 };
        }

        public static CounterState IncrementBy(CounterState state, IAction<int> amount)
        {
            return state with { value = state.value + amount.payload };
        }

        public static CounterState Reset(CounterState state, IAction action)
        {
